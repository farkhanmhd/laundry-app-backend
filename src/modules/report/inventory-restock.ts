// src/modules/report/inventory-restock.ts
//
// pdfkit PDF generator for the inventory restock report.
...
const COL_WIDTHS = [25, 110, 50, 50, 90, 100, 90]; // sum = 515
const ROW_HEIGHT = 20;
    ...
    if (items.length === 0) {
      ...
      items.forEach((item, idx) => {
        ...
        const cells = [
          ...
          { text: `+${item.restockQuantity}`, align: "center" },
        ...
        cells.forEach((cell, ci) => {
          doc
            .fillColor(ci === 2 ? "#2f855a" : TEXT_DARK)
